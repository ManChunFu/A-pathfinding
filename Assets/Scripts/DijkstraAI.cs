...
public class DijkstraAI : MonoBehaviour, IPathFinding
{
    ...
    {
        float x = newNodePosition.x - _startPoint.x;
        if (x < 0)
            x *= -1;
        float y = newNodePosition.y - _startPoint.y;
        if (y < 0)
            y *= -1;
    ...
    {
        float x = newNodePosition.x - _endPoint.x;
        if (x < 0)
            x *= -1;
        float y = newNodePosition.y - _endPoint.y;
        if (y < 0)
            y *= -1;
                    ...
                    else
                    {
                        if (newNodePosition.x >= minX && newNodePosition.x <= maxX && newNodePosition.y >= minY && newNodePosition.y <= maxY)
                                ...
                                if (!NodeArea.Any(n => n.Position == newNodePosition))
                            {
                                ...
                                Instantiate(_node, newNodePosition - new Vector2(9f, 5f), Quaternion.identity);
    ...
    private void DrawPath(Node last)
    {
        Node current = last;
        ...
        {
            Instantiate(_seed, item, Quaternion.identity);
        }
        PathCompleted = true;
    }
}
